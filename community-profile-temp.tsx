    ...
    return (
      <ThemedBackground style={styles.container}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['left', 'right', 'bottom']}>
          <Header
            title="Fehler"
            subtitle="Profil nicht gefunden"
          />
          <View style={styles.errorContainer}>
            ...
            <ThemedText style={styles.errorText}>
              Bitte melde dich an, um dein Profil anzuzeigen.
            </ThemedText>
          </View>
        </SafeAreaView>
      </ThemedBackground>
    ...
    <ThemedBackground style={styles.container}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['left', 'right', 'bottom']}>
        <Header 
          ...
          subtitle={profile?.first_name ? `${profile.first_name} ${profile.last_name || ''}` : ''}
          showBackButton={true}
        />
        ...
        {profile && (
          <View style={styles.content}>
            {/* Profilkarte */}
            <ThemedView style={styles.profileCard} lightColor="#FFFFFF" darkColor="#1A2530">
            ...
            {/* Tab-Inhalt */}
            <ThemedView style={styles.tabContent} lightColor="#FFFFFF" darkColor="#1A2530">
              {loading ? (
                <View style={styles.loadingContainer}>
                  <ActivityIndicator size="small" color={theme.accent} />
                  <ThemedText style={styles.loadingText}>
                    {activeTab === ProfileTab.FOLLOWERS ? 'Follower werden geladen...' :
                    activeTab === ProfileTab.FOLLOWING ? 'Folgt wird geladen...' :
                ...
                <>
                  ...
                  {activeTab === ProfileTab.FOLLOWERS && (
                    followers.length === 0 ? (
                      <View style={styles.emptyContainer}>
                        <IconSymbol name="person.2" size={32} color={theme.tabIconDefault} />
                        <ThemedText style={styles.emptyText}>Du hast noch keine Follower</ThemedText>
                      </View>
                    ) : (
                      <FlatList
                        data={followers}
                  ...
                  {activeTab === ProfileTab.FOLLOWING && (
                    following.length === 0 ? (
                      <View style={styles.emptyContainer}>
                        <IconSymbol name="person.2" size={32} color={theme.tabIconDefault} />
                        <ThemedText style={styles.emptyText}>Du folgst noch niemandem</ThemedText>
                      </View>
                    ) : (
                      <FlatList
                        data={following}
                  ...
                  {activeTab === ProfileTab.POSTS && (
                    posts.length === 0 ? (
                      <View style={styles.emptyContainer}>
                        ...
                        <ThemedText style={styles.emptyText}>Du hast noch keine Beiträge erstellt</ThemedText>
                      </View>
                    ) : (
                      <FlatList
                        data={posts}
