-- Skript zur Verbesserung der Synchronisierung von Schlafeinträgen zwischen verknüpften Benutzern
-- Dieses Skript verwendet account_links anstelle von user_connections

-- 1. Überprüfen, ob die Tabelle 'sleep_entries' existiert
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Die Tabelle "sleep_entries" existiert nicht. Stelle sicher, dass deine App die richtige Tabelle verwendet.';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Die Tabelle "account_links" existiert nicht. Die Synchronisierung wird nicht funktionieren.';
  END IF;
END
$$;

-- 3. Stelle sicher, dass die sleep_entries Tabelle external_id und synced_at spalten hat
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE sleep_entries ADD COLUMN external_id UUID REFERENCES sleep_entries(id) ON DELETE SET NULL;
    RAISE NOTICE 'Spalte "external_id" zur Tabelle "sleep_entries" hinzugefügt.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE sleep_entries ADD COLUMN synced_at TIMESTAMP WITH TIME ZONE;
    RAISE NOTICE 'Spalte "synced_at" zur Tabelle "sleep_entries" hinzugefügt.';
  END IF;
END
$$;

-- 4. Funktion zum einmaligen Synchronisieren aller bestehenden Schlafeinträge zwischen verknüpften Benutzern
...
-- 5. Funktion zum Hinzufügen eines Schlafeintrags
...
-- 6. Funktion zum Aktualisieren eines Schlafeintrags und Synchronisieren mit allen verknüpften Benutzern
...
-- 7. Funktion zum Löschen eines Schlafeintrags und Synchronisieren mit allen verknüpften Benutzern
...
DO $$
BEGIN
  RAISE NOTICE 'Skript erfolgreich ausgeführt. Die Synchronisierung der Schlafeinträge zwischen verknüpften Benutzern wurde eingerichtet.';
END
$$; 