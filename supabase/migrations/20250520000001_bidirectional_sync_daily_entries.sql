-- 1. Aktualisierte Trigger-Funktion zum automatischen Synchronisieren neuer Alltag-Einträge (bidirektional)
...
-- 2. Aktualisierte Trigger-Funktion zum automatischen Synchronisieren aktualisierter Alltag-Einträge (bidirektional)
...
-- 4. Aktualisierte Funktion zum Synchronisieren aller bestehenden Alltag-Einträge (bidirektional)
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Wir rufen die Funktion sync_daily_entries_from_inviter_to_invitee auf
      PERFORM sync_daily_entries_from_inviter_to_invitee(p_user_id);
      RAISE NOTICE 'Synced entries from inviter % to current user (invitee)', v_linked_user_id;
    END IF;
  END LOOP;
  CLOSE v_linked_users_cursor;
