...
import { View, TextInput, TouchableOpacity, StyleSheet, Alert, Switch, ActivityIndicator } from 'react-native';
...
export const CreatePollPost: React.FC<CreatePollPostProps> = ({
  onPollCreated,
  onCancel
}) => {
  ...
  // Umfrage-Post erstellen
  const handleCreatePollPost = async () => {
    // Validierung
    if (!question.trim()) {
    ...
    try {
      ...
      // Erstelle einen Post vom Typ 'poll'
      const { data, error } = await createPost(
        question.trim(), // Wir verwenden die Frage als Inhalt des Posts
        isAnonymous,
        'poll',
        pollData,
        selectedTagIds
  ...
  return (
    <ThemedView style={styles.container} lightColor={theme.card} darkColor={theme.card}>
      ...
      <View style={styles.switchContainer}>
        <ThemedText style={styles.switchLabel}>Anonym posten</ThemedText>
        <Switch
          value={isAnonymous}
          onValueChange={setIsAnonymous}
          trackColor={{ false: '#E0E0E0', true: '#E57373' }}
          thumbColor={isAnonymous ? '#FF9F9F' : '#f4f3f4'}
        />
      </View>
      ...
      <TouchableOpacity
        ...
        onPress={handleCreatePollPost}
        disabled={isCreating}
      >
        {isCreating ? (
          <ActivityIndicator size="small" color="#FFFFFF" />
        ) : (
          <ThemedText style={styles.createButtonText}>Umfrage veröffentlichen</ThemedText>
        )}
      </TouchableOpacity>
    </ThemedView>
  );
