...
export const checkSupabasePermissions = async () => {
  try {
    ...
    // Tabellen-Berechtigungen prüfen
    const tables = ['profiles', 'user_settings', 'baby_info'];
    const results = {};

    for (const table of tables) {
      console.log(`Checking permissions for table: ${table}`);

      // SELECT-Berechtigung prüfen
      const { data: selectData, error: selectError } = await supabase
        .from(table)
        .select('id')
        .limit(1);

      results[`${table}_select`] = { success: !selectError, error: selectError };
      console.log(`SELECT permission for ${table}:`, !selectError ? 'OK' : selectError);

      // INSERT-Berechtigung prüfen (mit einem Dummy-Eintrag, der sofort wieder gelöscht wird)
      if (table === 'baby_info') {
        const testData = {
        ...
        results[`${table}_insert`] = { success: !insertError, error: insertError, data: insertData };
        console.log(`INSERT permission for ${table}:`, !insertError ? 'OK' : insertError);

        // Löschen des Test-Eintrags, falls er erstellt wurde
        if (insertData && insertData.length > 0) {
          ...
          console.log(`DELETE permission for ${table}:`, !deleteError ? 'OK' : deleteError);
        }
      }
    }
    ...
    console.error('Error checking Supabase permissions:', err);
