...
export const redeemInvitationCode = async (userId: string, invitationCode: string) => {
  try {
    ...
    if (!finalData) {
      ...
      if (allInvitations) {
        console.log('Checking all invitations for similar codes...');

        // Exakte Suche in allen Einladungen
        const exactMatches = allInvitations.filter(inv =>
          inv.invitation_code === cleanedCode);

        if (exactMatches.length > 0) {
          console.log('Found exact match in all invitations:', exactMatches[0]);
          finalData = exactMatches[0];

          // Direkt zur Aktualisierung weitergehen
          console.log('Using exact match from all invitations');
        } else {
        ...
        // Wenn immer noch kein Match gefunden wurde, suche nach ähnlichen Codes
        if (!finalData) {
        ...
        // Generischer Fehler, wenn keine Einladungen gefunden wurden
        return {
          success: false,
          error: { message: `Keine Einladungen in der Datenbank gefunden. Bitte kontaktieren Sie den Support.` }
        };
      }
    }
    ...
    try {
      // Methode 1: Direkte Aktualisierung mit RLS deaktiviert
      console.log('Method 1: Direct update with RLS disabled');
      ...
      if (updateError) {
        ...
        try {
          ...
          if (rpcError) {
            ...
            return { success: true, linkData: rpcData.data };
          }
        } catch (rpcException) {
          console.error('Method 2 exception:', rpcException);
        }

        // Methode 3: Direktes SQL mit RPC
        console.log('Method 3: Direct SQL update via RPC');
        try {
          const { data: sqlData, error: sqlError } = await supabase.rpc('direct_update_invitation', {
            p_invitation_id: finalData.id,
            p_user_id: userId
          });

          if (sqlError) {
            console.error('Method 3 failed:', sqlError);
          } else {
            console.log('Method 3 succeeded:', sqlData);
            return { success: true, linkData: sqlData };
          }
        } catch (sqlException) {
          console.error('Method 3 exception:', sqlException);
        }

        // Wenn alle Methoden fehlschlagen, geben wir einen Fehler zurück
        return {
          success: false,
