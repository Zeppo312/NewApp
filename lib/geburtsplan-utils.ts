...
export const generateAndDownloadPDF = async (babyIconBase64: string | null, setIsGeneratingPDF?: (value: boolean) => void, locale: BirthPlanLocale = 'de') => {
  ...
  try {
    ...
    if (data.structured_data) {
      ...
      <div class="columns"><div class="column left-column">${formatContentForHTMLLeftColumn(content, locale)}</div><div class="column right-column">${formatContentForHTMLRightColumn(content, locale)}</div></div>
