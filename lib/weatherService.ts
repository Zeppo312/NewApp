...
import {
  ...
  getBabyWeatherApiLanguage,
  translateBabyWeatherText,
  type BabyWeatherLocale,
...
export async function getWeatherByCoordinates(
  lat: number,
  lon: number,
  locale: BabyWeatherLocale = DEFAULT_BABY_WEATHER_LOCALE,
): Promise<WeatherData> {
  const url = `${API_ENDPOINTS.WEATHER_BASE_URL}/weather?lat=${lat}&lon=${lon}&units=metric&lang=${getBabyWeatherApiLanguage(locale)}&appid=${API_KEYS.WEATHER_API_KEY}`;
  ...
  try {
    ...
    return transformWeatherData(data, locale);
  } catch (error) {
...
export async function getWeatherByZipCode(
  ...
  const url = `${API_ENDPOINTS.WEATHER_BASE_URL}/weather?zip=${zipCode},${countryCode}&units=metric&lang=${getBabyWeatherApiLanguage(locale)}&appid=${API_KEYS.WEATHER_API_KEY}`;
...
export async function getWeatherByCity(
  ...
  const url = `${API_ENDPOINTS.WEATHER_BASE_URL}/weather?q=${encodedCityName},${countryCode}&units=metric&lang=${getBabyWeatherApiLanguage(locale)}&appid=${API_KEYS.WEATHER_API_KEY}`;
 ...
 */
function transformWeatherData(data: any, locale: BabyWeatherLocale): WeatherData {
  ...
  return {
    ...
    description: data.weather[0]?.description || translateBabyWeatherText(locale, 'weather.noDescription'),
    icon: iconName,
...
export function getMockWeatherData(
  isDefaultLocation: boolean = true,
  ...
  if (isDefaultLocation) {
    return {
      temperature: 15,
      description: translateBabyWeatherText(locale, 'weather.mockPartlyCloudy'),
      icon: 'cloud.sun.fill',
      humidity: 65,
      windSpeed: 10,
      feelsLike: 14,
      location: translateBabyWeatherText(locale, 'location.currentLabel')
    };
  } else {
    return {
      temperature: 14,
      description: translateBabyWeatherText(locale, 'weather.mockCloudy'),
      icon: 'cloud.fill',
      humidity: 70,
      windSpeed: 8,
      feelsLike: 12,
      location: locationIdentifier
        ? (isCityName
          ? locationIdentifier
          : translateBabyWeatherText(locale, 'location.postalCode', { zipCode: locationIdentifier }))
        : translateBabyWeatherText(locale, 'location.unknown')
    };
  }
}
